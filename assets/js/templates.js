...
<script>
  ...
  roleAllow(id) {
    ...
    const map = {
      dashboard: all, about: all, contact: all, apply: all, notifications: all, academic_setup: admin, admin_data: admin, 'admin-data': admin, approvals: admin, storage: admin,
      ...
      academic_records: staff, cbt: staff, 'cbt-prompts': staff, 'cbt-exam': learner + ' parent', 'academic-records': staff,
  ...
  iconFor(id) {
    const map = {
      dashboard:'🏠', about:'🏫', contact:'☎️', apply:'📝', 'cbt-exam':'🧪', 'academic-records':'📄', academic_setup:'⚙️', students:'👨‍🎓', staff:'👨‍🏫', classes:'📚', attendance:'📋', results:'📊',
  ...
  labelFor(id, fallbackName) {
    const map = {
      dashboard:'Dashboard', about:'About', contact:'Contact', apply:'Apply', 'cbt-exam':'Take Exam', 'student-profile':'Student Profile', 'academic-records':'Academic Records', academic_setup:'Academic Setup', students:'Students', staff:'Staff', classes:'Classes',
  ...
  allModules(config) {
    ...
    const dedicatedPages = ['student-profile','cbt-exam','academic-records','about','contact','apply','notifications'];
    ...
    return T.shell(config, 'Dashboard', `
      ...
      <section data-dash-role="super_admin admin" style="display:none">
        ...
        <div class="grid grid-2">
          ...
          <div class="card"><h3>💰 Finance, HR & Operations</h3><p>Track payments, fee balances, payroll, loans, bonuses, inventory, transport, facilities, compliance and storage.</p><div style="display:flex;gap:8px;flex-wrap:wrap">${buttons(adminLinks.slice(9,18).concat(adminLinks.slice(22,28)))}</div></div>
          <div class="card"><h3>👨‍🏫 Staff / Teacher Oversight</h3><p>Admin can supervise teacher work without becoming a teacher: attendance coverage, results entry, CBT activity, lesson plans, scheme-of-work progress, messages and class records.</p><div style="display:flex;gap:8px;flex-wrap:wrap">${buttons(staffLinks)}</div></div>
          <div class="card"><h3>👨‍👩‍👧 Parent & Payment Oversight</h3><p>Admin can monitor parent-facing duties: linked children, payment tracking, fee balances, complaints, applications, parent messages and communication follow-up.</p><div style="display:flex;gap:8px;flex-wrap:wrap">${buttons(parentLinks.concat([['All Parents','parents.html'],['Finance Summary','finance.html'],['Approvals','approvals.html']]))}</div></div>
          <div class="card"><h3>🎓 Student Learning Oversight</h3><p>Admin can inspect the student experience for supervision: learning resources, assignments, timetable, results, report cards, profiles, certificates, diaries and support needs.</p><div style="display:flex;gap:8px;flex-wrap:wrap">${buttons(studentLinks.concat([['All Students','students.html'],['Promotion','promotion.html'],['Behaviour','behaviour.html'],['Support Plans','support_plans.html']]))}</div></div>
          <div class="card"><h3>🧭 Oversight Principle</h3><p>Staff, parents and students still see only their own dashboards. Admin and Super Admin see the admin dashboard plus controlled overviews of other dashboards for management, supervision, fee tracking and safeguarding.</p><div style="display:flex;gap:8px;flex-wrap:wrap"><a class="btn btn-outline btn-sm" href="analytics.html">Analytics</a><a class="btn btn-outline btn-sm" href="activity_log.html">Audit Trail</a><a class="btn btn-outline btn-sm" href="student-profile.html">Open Student Profile</a></div></div>
        </div>
      </section>
